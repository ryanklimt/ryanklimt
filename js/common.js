...
$(document).ready(function () {
    ...
    if ($('html').hasClass('ie8') || $('html').hasClass('ie7')) {
        var ieHomeLoopTimeout;
        ieHomeSlider(5000)
    }
    var workLoopTimeout;
    workSliders(4000, 300);
...
function labelPlaceholders() {
    $('.label-placeholders input[type=text], .label-placeholders input[type=password], .label-placeholders textarea').each(function () {
        if ($(this).val()) {
            $(this).parent('li').find('label').hide()
        }
    });
    $('.label-placeholders input[type=text], .label-placeholders input[type=password], .label-placeholders textarea').focusin(function () {
        $(this).parent('li').find('label').addClass('dim');
        $(this).keypress(function () {
            $(this).parent('li').find('label').hide()
        })
    });
    $('.label-placeholders input[type=text], .label-placeholders input[type=password], .label-placeholders textarea').focusout(function () {
        if (!$(this).val()) {
            ...
            $(form).fadeOut(300, function () {
                ...
                $(form).ajaxSubmit({
                    ...
                    success: function () {
                        ...
                        $('#message').html("<h2>Thanks! We've recieved your message.</h2>").append("<p>We will be in touch soon.</p>")
                    }
                })
...
function ieHomeSlider(interval) {
    $('.slide', slideshow).hide();
    $(window).load(function () {
        slideshow.removeClass('loading');
        $('.slide:first', slideshow).show().addClass('current');
        var length = $('.slide', slideshow).length;
        if (length > 1) {
            setTimeout(function () {
                ieHomeSliderLoop(interval)
            }, interval)
        }
    })
}

function ieHomeSliderLoop(interval) {
    var current = $('.slide.current', slideshow);
    current.hide();
    var next = current.next('.slide');
    if (next.is(':last')) {
    ...
    ieHomeLoopTimeout = setTimeout(function () {
        ieHomeSliderLoop(interval)
    }, interval)
}

function workSliders(interval, speed) {
    $('.iphone, .ipad, .iphoneV').each(function () {
