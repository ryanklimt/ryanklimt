	...
	a.fn.ajaxSubmit = function (c) {
		function r(d) {
			function w(c) {
				...
				var d = k.contentWindow ? k.contentWindow.document : k.contentDocument ? k.contentDocument : k.document;
				if (!d || d.location.href == g.iframeSrc) {
					if (!p) return
				}
				k.detachEvent ? k.detachEvent("onload", w) : k.removeEventListener("load", w, false);
				var e = "success",
					f;
				try {
					if (p) {
					...
					var i = g.dataType == "xml" || d.XMLDocument || a.isXMLDoc(d);
					b("isXml=" + i);
					if (!i && window.opera && (d.body == null || d.body.innerHTML == "")) {
						if (--u) {
					...
					m.responseText = l ? l.innerHTML : null;
					m.responseXML = d.XMLDocument ? d.XMLDocument : d;
					if (i) g.dataType = "xml";
					...
					if (l) {
						m.status = Number(l.getAttribute("status")) || m.status;
						m.statusText = l.getAttribute("statusText") || m.statusText
					}
					var n = g.dataType || "";
					var o = /(json|script|text)/.test(n.toLowerCase());
					if (o || g.textarea) {
						var r = d.getElementsByTagName("textarea")[0];
						if (r) {
							m.responseText = r.value;
							m.status = Number(r.getAttribute("status")) || m.status;
							m.statusText = r.getAttribute("statusText") || m.statusText
						} else if (o) {
							var t = d.getElementsByTagName("pre")[0];
							var y = d.getElementsByTagName("body")[0];
							if (t) {
								m.responseText = t.textContent ? t.textContent : t.innerHTML
							} else if (y) {
								m.responseText = y.innerHTML
							}
						}
					} else if (g.dataType == "xml" && !m.responseXML && m.responseText != null) {
						m.responseXML = x(m.responseText)
					}
					try {
						s = z(m, g.dataType, g)
					} catch (c) {
				...
				if (m.status) {
					e = m.status >= 200 && m.status < 300 || m.status === 304 ? "success" : "error"
				}
				if (e === "success") {
					g.success && g.success.call(g.context, s, "success", m);
					h && a.event.trigger("ajaxSuccess", [m, g])
				} else if (e) {
					if (f == undefined) f = m.statusText;
					g.error && g.error.call(g.context, m, e, f);
					h && a.event.trigger("ajaxError", [m, g, f])
				}
				h && a.event.trigger("ajaxComplete", [m, g]);
				if (h && !--a.active) {
					a.event.trigger("ajaxStop")
				}
				g.complete && g.complete.call(g.context, m, e);
				v = true;
				if (g.timeout) clearTimeout(q);
				setTimeout(function () {
					if (!g.iframeTarget) j.remove();
					m.responseXML = null
				}, 100)
			}
			function r() {
				...
				try {
					if (g.extraData) {
						for (var f in g.extraData) {
							d.push(a('<input type="hidden" name="' + f + '" value="' + g.extraData[f] + '" />').appendTo(e)[0])
						}
					}
					if (!g.iframeTarget) {
						j.appendTo("body");
						k.attachEvent ? k.attachEvent("onload", w) : k.addEventListener("load", w, false)
					}
					e.submit()
				} finally {
			...
			if (d) {
				for (f = 0; f < d.length; f++) {
					a(e[d[f].name]).attr("disabled", false)
				}
			}
			if (a(":input[name=submit],:input[id=submit]", e).length) {
			...
			if (g.iframeTarget) {
				j = a(g.iframeTarget);
				o = j.attr("name");
				if (o == null) j.attr("name", i);
				else i = o
			} else {
				j = a('<iframe name="' + i + '" src="' + g.iframeSrc + '" />');
				j.css({
					position: "absolute",
					top: "-1000px",
					left: "-1000px"
			...
			m = {
				...
				abort: function (c) {
					...
					b("aborting upload... " + d);
					this.aborted = 1;
					j.attr("src", g.iframeSrc);
					m.error = d;
					g.error && g.error.call(g.context, m, d, d);
					h && a.event.trigger("ajaxError", [m, g, d]);
					g.complete && g.complete.call(g.context, m, d)
				}
			};
			h = g.global;
			if (h && !(a.active++)) {
			...
			if (h) {
				a.event.trigger("ajaxSend", [m, g])
			}
			if (g.beforeSend && g.beforeSend.call(g.context, m, g) === false) {
				if (g.global) {
					a.active--
				}
				return
			...
			if (n) {
				...
				if (o && !n.disabled) {
					...
					g.extraData[o] = n.value;
					if (n.type == "image") {
						g.extraData[o + ".x"] = e.clk_x;
						g.extraData[o + ".y"] = e.clk_y
					}
				}
			}
			if (g.forceSync) {
		...
		var e = typeof d === "string" ? a.trim(d) : "";
		...
		if (c.beforeSubmit && c.beforeSubmit(i, this, c) === false) {
		...
		var q = l.attr("enctype") == p || l.attr("encoding") == p;
		if (c.iframe !== false && (o || c.iframe || q)) {
			if (c.closeKeepAlive) {
	...
	a.extend(a.fn, {
		validate: function (b) {
			if (this.length) {
				...
				return d
			} else b && b.debug && window.console && console.warn("nothing selected, can't validate, returning nothing")
		},
		valid: function () {
		...
		rules: function (b, d) {
			...
			if (b) {
				...
				case "add":
					...
					g[e.name] = h;
					if (d.messages) f.messages[e.name] = a.extend(f.messages[e.name], d.messages);
					break;
				case "remove":
					if (!d) {
						delete g[e.name];
			...
			e = a.validator.normalizeRules(a.extend({}, a.validator.metadataRules(e), a.validator.classRules(e), a.validator.attributeRules(e), a.validator.staticRules(e)), e);
			if (e.required) {
				f = e.required;
				delete e.required;
				e = a.extend({
	...
	a.extend(a.validator, {
		...
		prototype: {
			...
			check: function (b) {
				...
				for (f in d) {
					...
					try {
						...
						if (h == "dependency-mismatch") e = true;
						else {
							e = false;
							if (h == "pending") {
						...
						this.settings.debug && window.console && console.log("exception occured when checking element " + b.id + ", check the '" + g.method + "' method", i);
						throw i
					}
				}
				if (!e) {
			...
			formatAndAdd: function (a, b) {
				var c = this.defaultMessage(a, b.method),
					d = /\$?\{(\d+)\}/g;
				if (typeof c == "function") c = c.call(this, b.parameters, a);
				else if (d.test(c)) c = jQuery.format(c.replace(d, "{$1}"), b.parameters);
				this.errorList.push({
					message: c,
					element: a
				});
			...
			showLabel: function (b, d) {
				...
				if (e.length) {
					...
					e = a("<" + this.settings.errorElement + "/>").attr({
						"for": this.idOrName(b),
						generated: true
					}).addClass(this.settings.errorClass).html(d || "");
					if (this.settings.wrapper) e = e.hide().show().wrap("<" + this.settings.wrapper + "/>").parent();
					this.labelContainer.append(e).length || (this.settings.errorPlacement ? this.settings.errorPlacement(e, a(b)) : e.insertAfter(b))
				}
		...
		attributeRules: function (b) {
			...
			for (var e in a.validator.methods) {
				var f;
				if (f = e === "required" && typeof a.fn.prop === "function" ? b.prop(e) : b.attr(e)) d[e] = f;
				else if (b[0].getAttribute("type") === e) d[e] = true
			}
		...
		staticRules: function (b) {
			var d = {},
				e = a.data(b.form, "validator");
			if (e.settings.rules) d = a.validator.normalizeRule(e.settings.rules[b.name]) || {};
			return d
		},
		normalizeRules: function (b, d) {
			a.each(b, function (e, f) {
				if (f === false) delete b[e];
					...
					if (g) b[e] = f.param !== undefined ? f.param : true;
					else delete b[e]
				}
			});
			a.each(b, function (e, f) {
		...
		addMethod: function (b, d, e) {
			...
			a.validator.messages[b] = e != undefined ? e : a.validator.messages[b];
		...
		methods: {
				...
				a.ajax(a.extend(true, {
					...
					success: function (e) {
						...
						if (h) {
							...
							g.prepareElement(d);
							g.formSubmitted = i;
							g.successList.push(d);
							g.showErrors()
						} else {
							i = {};
							e = e || g.defaultMessage(d, "remote");
							i[d.name] = f.message = a.isFunction(e) ? e(b) : e;
							g.showErrors(i)
						}
						f.valid = h;
						g.stopRequest(d, h)
			...
			rangelength: function (b, d, e) {
				...
				return this.optional(d) || b >= e[0] && b <= e[1]
			},
			min: function (a, b, c) {
			...
			accept: function (a, b, c) {
				...
				return this.optional(b) || a.match(RegExp(".(" + c + ")$", "i"))
			},
			equalTo: function (b, d, e) {
				e = a(e).unbind(".validate-equalTo").bind("blur.validate-equalTo", function () {
					a(d).valid()
				});
				return b == e.val()
			}
...
(function (a) {
	...
	if (a.ajaxPrefilter) a.ajaxPrefilter(function (a, c, d) {
		...
		a.ajax = function (d) {
			...
			if (("mode" in d ? d : a.ajaxSettings).mode == "abort") {
				b[e] && b[e].abort();
